...
def test_migrate_legacy_snapshot_copies_all_state():
    class Source:
        def load_servers_config(self): return {"servers": [{"id": "x"}]}
        def load_env_servers_config(self): return {"env_servers": {"prod": "x"}}
        def get_deployment_history(self): return [{"id": 1}]
        def load_env_container_bindings(self): return {"env_containers": {"dev": ["a"], "prod": ["b"]}}
    class Target:
        def __init__(self): self.calls = {}
        def save_servers_config(self, v): self.calls["servers"] = v
        def save_env_servers_config(self, v): self.calls["env"] = v
        def replace_deployment_history(self, v, max_entries=50): self.calls["history"] = v
        def save_env_container_bindings(self, v): self.calls["bindings"] = v
    target = Target()
    result = environment_state.migrate_legacy_file_state_to_store(
        target_store=target, file_store_factory=Source
    )
