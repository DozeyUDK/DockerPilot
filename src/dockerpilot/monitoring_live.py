"""Live streaming container monitoring extracted from MonitoringManager."""
...
def monitor_container_live(host: Any, container_name: str, duration: int = 30) -> bool:
    """Live monitoring with screen clearing (from dockerpilot-Lite)"""
    with host._error_handler(f"live monitor {container_name}", container_name):
        container = host.client.containers.get(container_name)

        host.console.print(f"[cyan]Starting live monitoring for {container_name} ({duration}s)...[/cyan]")
        host.console.print(f"[yellow]Press Ctrl+C to stop[/yellow]\n")
        ...
        try:
            for raw_stats in stats_stream:
                ...
                try:
                    ...
                    host.console.print(f"[bold cyan]📊 Live Monitoring: {container_name}[/bold cyan]")
                    host.console.print(f"[green]🖥️  CPU: {cpu_percent:.2f}%[/green]")
                    host.console.print(f"[blue]💾 RAM: {mem_usage:.1f}MB / {mem_limit:.1f}MB ({mem_percent:.1f}%)[/blue]")
                    host.console.print(f"[yellow]⏱️  Time: {int(current_time - start_time)}/{duration}s[/yellow]")
                    host.console.print(f"[dim]Press Ctrl+C to stop[/dim]")

                    prev_stats = stats

                except (json.JSONDecodeError, UnicodeDecodeError) as e:
                    host.logger.warning(f"Stats parsing error: {e}")
                    continue
                except Exception as e:
                    host.logger.warning(f"Stats processing error: {e}")
                    continue
